...
fn main() {
    ...
    let pairs = calc_pairs(img, &mut map, &mut weights, &mut inv_map);
    println!("{}", pairs.len());
    println!("{:?}", pairs);
...
fn generate(
    ...
    // all the waves
    let mut waves = vec![vec![map.keys().collect::<Vec<&Rgba<u8>>>()]];
    ...
    let out = DynamicImage::new_rgb8(width as u32, height as u32);
